...
const Card = ({ product, changeInterface }) => {
  ...
  const handleDetailsUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://smart-tech-l4lqlqntn-atta-un-nabis-projects.vercel.app/api/UpdateProduct', {
  ...
  const handelDelete = async () => {
    try {
      let response = await fetch('https://smart-tech-l4lqlqntn-atta-un-nabis-projects.vercel.app/api/DeleteProduct', {
        method: 'POST',
        headers: {
          'X-Content-Type-Options': 'nosniff',
          'Content-Type': 'application/json'
